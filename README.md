...
RustForge 面向「已获授权但不知从何下手」的渗透初学者：内置 MITM 代理抓取目标流量，云端 AI（**用户自带 Key**）解释接口、标记可疑点，并提出版本化**测试计划**一步步引导你操作；AI 变更需先审阅 diff 再由人工合并。每个结论都附带 OWASP/CWE 知识、置信度与**人工验证步骤**，配合内置 Repeater 手动复核，最后一键导出学习报告。
...
| **测试计划** | AI 基于流量、Finding、Evidence 和当前 revision 生成 proposal/diff；人工确认后事务合并；节点支持类型、priority、前置依赖、字段锁、预期/实际观察和六种状态；与 Finding/Evidence 双向关联；vue-flow 可视化 |
...
| **学习报告** | 一键导出 Markdown：发现列表（含复现与修复建议）+ 渗透过程时间线 + 测试计划概览 |
...
```
抓包(MITM) ─▶ 被动规则初筛 + AI 分析 ─▶ 测试计划引导 ─▶ Repeater 手动验证 ─▶ 发现确认 ─▶ 学习报告
```
...
│  流量表格 / 详情 / Repeater / 测试计划(vue-flow) /     │
│  发现 + 知识卡片 / 报告 / 设置                          │
├──────────────── Tauri Commands / Events ─────────────┤
│  Rust 后端 (tokio 异步)                                │
│  ├─ proxy   : hudsucker(hyper+rustls+rcgen) MITM       │
│  ├─ rules   : 被动规则引擎（regex/启发式）              │
│  ├─ ai      : LLM 客户端 + 提示词 + 分析器 + 计划提案   │
│  ├─ tree    : 版本化测试计划 + proposal 事务合并        │
...
5. 启动代理 → 浏览目标 → 流量实时出现 → 选中请求做 AI 分析 / 生成测试计划 proposal → 审阅 diff 并合并 → Repeater 验证 → 导出报告。
...
| Phase 3 | 测试计划（AI proposal/diff、版本、状态机、vue-flow 引导） | ✅ |
