...
> ## 授权与免责声明
>
> RustForge 仅用于对已获得明确书面授权的目标进行安全测试与学习。首次启动会要求确认授权声明；AI 只能从版本化 ToolSpec 中选择不透明 surface/resource 和已有参数，不能生成 URL、方法、Header、正文或 payload。目标请求由 Rust 后端同时执行 mission 契约、工具权限、`AssessmentPolicy` 与 `ScopePolicy`；只有确定性安全验证器可以自动确认，模型输出本身永远不能确认漏洞。Scope 和运行契约是技术保护边界，不是法律授权的替代品。详细要求见 [docs/AUTHORIZATION.md](docs/AUTHORIZATION.md)。

RustForge 把“用户目标 → 任务分解 → 可信工具调用 → 分级审批 → 确定性验证 → 人工接力 → Evidence 报告”串成一条可追溯闭环。用户可以复用项目 Traffic、Finding、历史评估和 OpenAPI，在任务执行中继续追问或调整目标；它不是通用利用器，也不向模型开放任意网络能力。
...
| **AI 上下文防火墙** | API Key 存系统凭据库；发送前展示最终上下文；URL/Header/JSON/表单/multipart/高熵秘密结构化脱敏；预览哈希绑定 provider、模型、提示词、policy 和 Schema；模型输出经后端校验、证据强度校准与重复假设合并 |
| **版本化标准** | 离线知识包固定到 OWASP Top 10 2021/2025、OWASP API Top 10 2023、ASVS 5.0.0、WSTG 4.2、CWE 4.20；未知版本或编号不会被猜测映射 |
| **目标驱动 AI 评估 v2** | 目标对话、两层工作流、版本化 ToolSpec、项目资源、上下文披露确认、三档预算、分级审批、中途引导、持久化等待/恢复；网络层仍固定并发 1、精确 origin、2 RPS 硬上限、无正文和无自动重定向 |
...
| **Repeater 与人工接力** | 项目内多会话、TLS 策略、不可变发送历史、失败/越界审计、分页和 run diff；高风险配方只创建草稿，必须由用户点击发送，回传结果默认是待复核 Evidence |
| **旧运行与测试计划** | 固定轮次 Phase 6 run 作为只读 legacy mission；原 proposal/revision/task tree 数据和 API 隐藏兼容，不接收 Mission 写入，也不再被描述为已执行测试 |
| **证据化报告 v4** | 同时导出 Markdown 与 JSON；新增目标、附件、工作流、工具/权限 manifest、审批、动作、人工接力、成本和覆盖矩阵；旧 run 报告继续使用 v3 并明确 legacy |
...
- 不引入任意 Shell、浏览器利用、多 Agent 并行攻击、自动修复或并行目标请求；人工配方永远不能自动发送。
- 当前不记录 WebSocket 消息，也不提供 SSE/gRPC/GraphQL 专用语义或自动重定向。
...
目标 + Scope + 项目资源
          │
          ▼
 脱敏上下文确认 ─▶ 工作流 / ToolSpec 动作 ─▶ 后端权限与审批
                                                    │
                         ┌──────────────────────────┴─────────────┐
                         ▼                                        ▼
             非破坏探针 + 确定性验证                  人工 Repeater 草稿
                         │                              用户点击发送 │
                         └───────────────────────┬─────────────────┘
                                                 ▼
             confirmed / suspected / not observed / coverage gap
                                                 │
                                                 ▼
                   脱敏 Evidence + 审批轨迹 ─▶ Report Schema v4
```
...
│ ai             脱敏上下文 + 提示词版本 + 输出校验            │
│ assessment     Mission / ToolSpec / 审批 / surface / 验证器  │
│ replay         手动与评估隔离的 session / attempt / run      │
│ evidence       快照、人工/验证器接受和 Finding 事件          │
│ tree           隐藏兼容的旧测试计划模块                      │
│ knowledge      版本化离线标准包                              │
│ report         Mission Report Schema v4 + legacy v3           │
│ storage        SQLite v4 + 迁移备份/外键/触发器约束           │
└─────────────────────────────────────────────────────────────┘
```
...
- [Rust](https://www.rust-lang.org/) 1.93.0（`rust-toolchain.toml` 固定日常工具链）
...
cargo +1.93.0 check --manifest-path src-tauri/Cargo.toml --all-targets
```
...
3. 在“AI 评估”描述目标并填写起始 URL；可选附加 Traffic、Finding、历史评估或有界 OpenAPI，配置身份、预算档位和权限模式。
4. 如需 AI，在设置中添加 OpenAI 兼容 provider；API Key 和身份值写入系统凭据库，而非 SQLite 或前端持久状态。
5. 审阅精确 origin、身份、ToolSpec/权限快照和最终 AI 数据披露；上下文 hash 确认后启动任务。
6. 在任务对话中查看理由、预期观察、风险、审批与结果；可继续追问，异常时停止并保留部分结果。
7. 在结果页区分已确认、疑似、未观察到和覆盖缺口，并查看脱敏 Replay/Evidence。
8. 高风险类别须先获批，再由 AI 在 allowlist 内选择具体不透明 surface/参数并经后端复核；随后也只生成独立 Repeater 草稿。用户亲自点击发送后，可把同 handoff 的 ReplayRun 回传为待复核 Evidence。
9. 按 mission 导出默认脱敏的 Schema v4 Markdown/JSON；旧 run 继续使用 legacy v3 报告。
...
│  ├─ stores/                   assessmentMission / project / traffic / replay / findings
...
│  ├─ src/assessment/           Mission、ToolSpec、发现、策略、DSL 与验证
│  ├─ src/replay/               手动/评估隔离的会话、run 与 diff
│  ├─ src/evidence/             Evidence 与 Finding 复核服务
│  ├─ src/tree/                 隐藏兼容的版本化测试计划
│  ├─ src/knowledge/            版本化离线标准包
│  ├─ src/report.rs             Mission Report Schema v4 与 legacy v3
│  └─ src/storage/              SQLite v4 schema、迁移备份与模型
...
| Phase 6 | 目标驱动 Mission、可信工具与审批、人工接力、会话恢复和报告 v4 | 完成 |
...
> 核查日期：2026-08-03。以下链接和定位按当日官方文档或仓库 README 复核；再次据此做架构决策前必须重新核查。参考不代表依赖、兼容或复制其自主执行模型。

| 项目 | RustForge 采用的定位 | 明确不继承的部分 |
|---|---|---|
| [Burp AT](https://portswigger.net/burp/documentation/desktop/burp-at) / [Repeater](https://portswigger.net/burp/documentation/desktop/tools/repeater) | 项目任务、资源附件、工具权限、动作追踪，以及人工重放和对比交互基准 | 不引入任意工具、自主利用或绕过逐请求后端策略 |
...
| [PentestGPT v1.0](https://github.com/GreyDGL/PentestGPT) / [USENIX 论文](https://www.usenix.org/system/files/usenixsecurity24-deng.pdf) | 阶段化任务分解、可解释检查和会话恢复 | 不继承通用自主 agent；RustForge 的模型只能选择后端版本化工具与不透明 surface |
...
| [Strix](https://github.com/usestrix/strix) | 运行可视化、过程追踪、实时引导和证据表达 | 不引入多 Agent 攻击、自动 PoC、自动利用或自动修复 |
