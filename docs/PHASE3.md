...
> 历史兼容说明，最后复核：2026-08-01。Phase 3 交付的文字测试计划、revision 和事件仍完整保存在 SQLite v3，但 `/tasks` 已由 [Phase 6 AI Assessment](PHASE6.md) 替换。该模块从主导航隐藏，不接收 Assessment 写入；以下内容仅描述兼容 API 与既有数据语义。
...
- 旧 tree planner 仍只规划和解释，不会触发网络；Phase 6 的网络执行属于独立 Assessment 领域，并受固定模板和双策略约束。
