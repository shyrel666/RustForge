...
impl OpenAiClient {
    ...
    async fn chat_once(&self, system: &str, user: &str) -> Result<ChatResponse, CallError> {
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| CallError::Retryable(format!("请求 LLM 失败: {e}")))?;

        let status = resp.status();
        let text = resp.text().await.map_err(|e| CallError::Retryable(e.to_string()))?;
        if !status.is_success() {
            // 把服务端错误信息透传给用户（如余额不足/模型名错误）
            let snippet: String = text.chars().take(300).collect();
            let msg = format!("LLM API 返回 {status}: {snippet}");
        ...
        let content = json["choices"][0]["message"]["content"]
            ...
            .ok_or_else(|| {
                let snippet: String = text.chars().take(300).collect();
                CallError::Fatal(format!("LLM 响应结构异常: {snippet}"))
            })?;
        let usage = Usage {
...
impl LlmClient for OpenAiClient {
    async fn chat(&self, system: &str, user: &str) -> Result<ChatResponse, String> {
        match self.chat_once(system, user).await {
            Ok(c) => Ok(c),
            Err(CallError::Retryable(e)) => {
                eprintln!("[ai] 首次调用失败，1s 后重试: {e}");
                tokio::time::sleep(Duration::from_secs(1)).await;
                self.chat_once(system, user).await.map_err(|e| match e {
